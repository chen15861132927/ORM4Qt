 ...
 */
template<class T>
std::shared_ptr<QSqlRelationalTableModel> listToQtModel(const QList<std::shared_ptr<T>>& list)
{
	...
	filter += ')';
	std::shared_ptr<QSqlRelationalTableModel> model = std::make_shared<QSqlRelationalTableModel>();
